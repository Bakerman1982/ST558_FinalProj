...
#I understand we are not supposed to run the model with all main effects. I will exclude it from my terminal evaluations.  

base_model_rfPred <- predict(base_model_rf, newdata = dplyr::select(diabetes_test, -Diabetes_binary))

# Convert predictions to numeric
base_model_rfPred_numeric <- as.numeric(base_model_rfPred) - 1
diabetes_test_numeric <- as.numeric(diabetes_test$Diabetes_binary) - 1

# Calculate RMSE
base_model_rfRMSE <- sqrt(mean((base_model_rfPred_numeric - diabetes_test_numeric)^2))
print(base_model_rfRMSE)

c(rf = base_model_rfRMSE, bag = bagRMSE, tree = treeRMSE)
